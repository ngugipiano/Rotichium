...
class UserPackageController extends AdminController
{
    ...
    public function package_purchase_history(Request $request)
    {
        ...
        if ($cate) {
           $package_payments->where('cat_id', $cate);
        }
        if ($post_name) {
            $package_payments->where('title', 'LIKE', '%' . $post_name . '%');
            $package_payments->orderBy('title', 'asc');
        }
        ...
        $data = [
            'rows'        => $package_payments->with("getAuthor")->with("getCategory")->paginate(20),
