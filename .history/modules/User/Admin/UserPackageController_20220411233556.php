...
class UserPackageController extends AdminController
{
    ...
    public function package_purchase_history(Request $request)
    {
        ...
        $package_payments = PackagePayment::query()->orderBy('id', 'desc');
        $user_name = User::where('name', $request->query('s'));
        
        if ($package_name) {
            $package_payments->where('name', 'LIKE', '%' . $user_name . '%');
            $package_payments->orderBy('name', 'asc');
        }

        // $this->filterLang($package_payments);

        $data = [
            'rows'        => $package_payments->paginate(20),
            'breadcrumbs' => [
                [
                    'name' => __('User Packages'),
                    'url'  => route('user_packages.index')
                ...
                [
                    'name'  => __('Package Payment History'),
                    'class' => 'active'
                ],
            ],
            "languages"=>Language::getActive(false),
            "locale"=>\App::getLocale(),
            'page_title'=>__("Package Payment History")
        ];
        return view('User::admin.packages.payment_history', $data);
    }
